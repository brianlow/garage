...
ResponsiveFilter::ResponsiveFilter(bool sleepEnable, int maxValue, int minValue, float snapMultiplier)
{
...
void ResponsiveFilter::update(int rawValue)
{
...
int ResponsiveFilter::getResponsiveValue(int newValue)
{
  ...
  if(sleepEnable && edgeSnapEnable) {
    if(newValue < minValue + activityThreshold) {
      newValue = (newValue * 2) - activityThreshold;
    } else if(newValue > maxValue - activityThreshold) {
      newValue = (newValue * 2) - maxValue + activityThreshold;
    }
  }
  ...
  if(smoothValue < minValue) {
    smoothValue = minValue;
  } else if(smoothValue > maxValue - 1) {
    smoothValue = maxValue - 1;
  }
...
float ResponsiveFilter::snapCurve(float x)
{
  float y = 1.0 / (x + 1.0);
  y = (1.0 - y) * 2.0;
  if(y > 1.0) {
...
void ResponsiveFilter::setSnapMultiplier(float newMultiplier)
{
